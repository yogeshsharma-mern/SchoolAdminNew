...
═══════════════════════════════════════════════ */
function ClassModal({ isOpen, onClose, title, subtitle, children, maxWidth = 520 }) {
  useEffect(() => {
  ...
  return (
    <div
      style={{
        ...
        padding: 16,
        background: cva('text', .45), backdropFilter: 'blur(1px)',
...
/* ═══════════════════════════════════════════════
   react-select theme builder (reads CSS vars)
═══════════════════════════════════════════════ */
function buildSelectStyles(hasError = false) {
  return {
    control: (base, state) => ({
      ...
      boxShadow: state.isFocused && !hasError
        ? `0 0 0 3px ${cva('primary', 0.14)}`
        : 'none',
      transition: 'all .15s',
      cursor: 'pointer',
      '&:hover': {
        borderColor: hasError ? cv('danger') : cv('primary'),
      },
    }),
    menu: (base) => ({
      ...base,
      ...
      border: `1px solid ${cv('border')}`,
      borderRadius: 14,
      boxShadow: '0 20px 60px -8px rgba(0,0,0,.35)',
      overflow: 'hidden',
      zIndex: 9999,
    }),
    menuList: (base) => ({
    ...
    option: (base, state) => ({
      ...
      backgroundColor: state.isSelected
        ...
        : state.isFocused
          ? cva('primary', 0.07)
          : 'transparent',
      color: state.isSelected ? cv('primary') : cv('text'),
      cursor: 'pointer',
    ...
    multiValue: (base) => ({
      ...base,
      backgroundColor: cva('primary', 0.12),
      borderRadius: 8,
      border: `1px solid ${cva('primary', 0.25)}`,
      padding: '1px 2px',
    }),
    ...
    multiValueRemove: (base) => ({
      ...
      cursor: 'pointer',
      '&:hover': {
        backgroundColor: cva('danger', 0.15),
        color: cv('danger'),
      },
    }),
    input: (base) => ({
...
/* ═══════════════════════════════════════════════
   ADD SUBJECTS MODAL
═══════════════════════════════════════════════ */
function AddSubjectsModal({ isOpen, onClose, classItem, allSubjects, isLoadingSubjects }) {
  const queryClient = useQueryClient();
  const [selectedSubjects, setSelectedSubjects] = useState([]);
  const [subjectError, setSubjectError] = useState('');

  // Reset when modal opens/changes class
  useEffect(() => {
    if (isOpen) {
      setSelectedSubjects([]);
      setSubjectError('');
    }
  }, [isOpen, classItem?._id]);

  // Build react-select options from subjects list
  const subjectOptions = useMemo(() =>
    (allSubjects?.results?.docs || []).map(s => ({
      value: s._id,
      label: s.name,
  ...
  // Subject emoji for option display
  const subjectEmojis = {
    Math: '📐', Science: '🔬', Hindi: '📖', Sanskrit: '🕉️',
    English: '📝', SST: '🌏', Sst: '🌏', Physics: '⚡',
    Chemistry: '🧪', Biology: '🧬', GKS: '🌟', Gks: '🌟',
  };

  const getEmoji = (name) => subjectEmojis[name] || '📚';

  const addSubjectsMutation = useMutation({
    mutationFn: ({ classId, subjectIds }) =>
      apiPut(`${apiPath.addSubjectsToClass}/${classId}`, { subjectIds }),
    onSuccess: (r) => {
      toast.success(r?.message || 'Subjects added successfully!');
      queryClient.invalidateQueries({ queryKey: ['classes'] });
      onClose();
    },
    onError: (e) => {
      toast.error(e?.response?.data?.message || 'Failed to add subjects');
    },
  });
  ...
  const handleSubmit = () => {
    ...
    setSubjectError('');
    const classId = classItem?.classId || classItem?._id;
    const subjectIds = selectedSubjects.map(s => s.value);
    addSubjectsMutation.mutate({ classId, subjectIds });
  };
  ...
  return (
    <ClassModal
      ...
      subtitle={`Adding subjects to ${className}`}
      maxWidth={560}
    >
      {/* Class badge */}
      <div style={{
        ...
        background: cva('primary', 0.07),
        border: `1px solid ${cva('primary', 0.18)}`,
        marginBottom: 22,
      }}>
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: "'Sora',sans-serif", fontWeight: 900, fontSize: 14,
        }}>
          {className}
        </div>
        <div>
          <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 800, fontSize: 14, color: cv('text') }}>
            Class {className}
          </div>
          <div style={{ fontSize: 12, color: cv('muted'), marginTop: 2 }}>
        ...
        {selectedSubjects.length > 0 && (
          <div style={{
            ...
            background: cv('primary'), color: cv('surface'),
            fontSize: 11.5, fontWeight: 800, fontFamily: "'Sora',sans-serif",
          }}>
      ...
      <Field
        ...
        required
        error={subjectError}
        hint="You can select multiple subjects at once"
      >
        <ReactSelect
          isMulti
          ...
          onChange={(selected) => {
            ...
            if (selected?.length) setSubjectError('');
          }}
          styles={buildSelectStyles(!!subjectError)}
          placeholder={isLoadingSubjects ? 'Loading subjects…' : 'Search and select subjects…'}
          ...
          formatOptionLabel={(opt) => (
            <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
              <span style={{ fontSize: 16 }}>{getEmoji(opt.label)}</span>
              <div>
                <div style={{ fontWeight: 600, fontSize: 13, color: cv('text') }}>{opt.label}</div>
                {opt.description && (
                  <div style={{ fontSize: 11, color: cv('muted'), marginTop: 1 }}>
                    {opt.description}
                  </div>
                )}
              </div>
      ...
      {selectedSubjects.length > 0 && (
        <div style={{
          ...
          background: cva('success', 0.06),
          border: `1px solid ${cva('success', 0.18)}`,
          marginBottom: 20,
        }}>
          <div style={{
            fontSize: 10.5, fontWeight: 700, textTransform: 'uppercase',
            letterSpacing: '.08em', color: cv('success'), marginBottom: 9,
            display: 'flex', alignItems: 'center', gap: 6,
          }}>
            <I.Check width={11} height={11} /> Ready to assign — {selectedSubjects.length} subject{selectedSubjects.length > 1 ? 's' : ''}
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
            {selectedSubjects.map(s => (
              <span key={s.value} style={{
                ...
                padding: '4px 10px', borderRadius: 20,
                background: cva('primary', 0.1),
                border: `1px solid ${cva('primary', 0.2)}`,
                fontSize: 11.5, fontWeight: 700,
                color: cv('primary'), fontFamily: "'Sora',sans-serif",
              }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
        <button
          ...
          onMouseEnter={e => e.currentTarget.style.background = cv('surface')}
          onMouseLeave={e => e.currentTarget.style.background = cv('bg')}
        >
          Cancel
        </button>
        <button
          ...
          style={{
            ...
            background: addSubjectsMutation.isPending ? cva('primary', 0.6) : cv('primary'),
            color: cv('surface'),
            fontSize: 13.5, fontWeight: 700,
            cursor: addSubjectsMutation.isPending ? 'not-allowed' : 'pointer',
            fontFamily: "'Sora',sans-serif",
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
            boxShadow: addSubjectsMutation.isPending ? 'none' : `0 6px 20px -4px ${cva('primary', 0.45)}`,
            transition: 'all .15s',
          }}
          onMouseEnter={e => { if (!addSubjectsMutation.isPending) e.currentTarget.style.filter = 'brightness(1.07)'; }}
          onMouseLeave={e => { e.currentTarget.style.filter = 'none'; }}
        >
          {addSubjectsMutation.isPending ? (
            <><I.Spinner width={14} height={14} /> Assigning…</>
          ) : (
            <><I.Book width={14} height={14} /> Assign {selectedSubjects.length > 0 ? `${selectedSubjects.length} ` : ''}Subjects</>
          )}
        </button>
      </div>
    </ClassModal>
...
export default function Classes() {
  ...
  const [subjectTargetClass, setSubjectTargetClass] = useState(null);
  ...
  // Fetch all subjects for the multi-select
  ...
  const handleOpenAddSubjects = (item) => {
    setSubjectTargetClass(item);
    setShowAddSubjects(true);
  };
  const handleCloseAddSubjects = () => {
    setShowAddSubjects(false);
    setSubjectTargetClass(null);
  };

  // ── Navigate to class subjects view ──
  const handleViewSubjects = (item) => {
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              <div style={{ fontWeight: 700, fontSize: 13.5, color: cv('text'), fontFamily: "'Sora',sans-serif" }}>
                {isNaN(v) || ['Prep', 'PreKG', 'KG'].includes(v) ? v : `${v}${['1', '2', '3'].includes(v) ? ['st', 'nd', 'rd'][+v - 1] : 'th'}`}
              </div>
      ...
      cell: ({ row }) => {
        ...
        return (
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
            ...
            {/* Delete */}
            <ActionBtn
              icon={I.Trash} label="Delete"
              colorVar="danger"
              onClick={() => handleDelete(item)}
            />
            {/* Add Subjects */}
            <ActionBtn
              icon={I.Book} label="Add Subjects"
              colorVar="success"
              onClick={() => handleOpenAddSubjects(item)}
            />
            {/* View Subjects */}
            <ActionBtn
              icon={I.Eye}
  ...
  return (
    <>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
            ...
            <button
              ...
              style={{
                ...
                color: cv('surface'), fontSize: 13.5, fontWeight: 700,
      ...
      {/* ══ Add Subjects Modal ══ */}
      <AddSubjectsModal
        isOpen={showAddSubjects}
        onClose={handleCloseAddSubjects}
        classItem={subjectTargetClass}
...
═══════════════════════════════════════════════ */
function ActionBtn({ icon: Icon, label, colorVar, onClick }) {
  return (
    <button
      onClick={onClick}
      title={label}
      style={{
        ...
        background: cva(colorVar, .1), color: cv(colorVar),
        fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
        transition: 'all .15s', whiteSpace: 'nowrap',
      }}
      onMouseEnter={e => {
        e.currentTarget.style.background = cva(colorVar, .2);
        e.currentTarget.style.transform = 'translateY(-1px)';
        e.currentTarget.style.boxShadow = `0 4px 12px -4px ${cva(colorVar, .35)}`;
      }}
      onMouseLeave={e => {
        e.currentTarget.style.background = cva(colorVar, .1);
