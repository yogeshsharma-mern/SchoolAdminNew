...
//               {showThemeMenu && (
//                 <div className="absolute right-[100%] mt-2 w-80 bg-[rgb(var(--color-surface))] rounded-2xl shadow-2xl border border-[rgb(var(--color-border))] overflow-hidden animate-scaleIn z-50">
//                   {/* Header */}
...
const ModernHeader = () => {
  ...
  const themes = [
    { id: 'light', name: 'Light', icon: Sun, primary: '#2563eb', secondary: '#16a34a' },
    { id: 'dark', name: 'Dark', icon: Moon, primary: '#3b82f6', secondary: '#22c55e' },
    { id: 'sunset', name: 'Sunset', icon: Palette, primary: '#f97316', secondary: '#f43f5e' },
    { id: 'forest', name: 'Forest', icon: Palette, primary: '#22c55e', secondary: '#10b981' },
    { id: 'amber', name: 'Amber', icon: Palette, primary: '#f59e0b', secondary: '#fbbf24' },
    { id: 'neon', name: 'Neon', icon: Palette, primary: '#14b8a6', secondary: '#22d3ee' },
    { id: 'carbon', name: 'Carbon', icon: Palette, primary: '#4b5563', secondary: '#6b7280' }
  ];
  ...
  return (
    <header className={`
      ...
      ${isScrolled 
        ? 'shadow-lg border-b border-[rgb(var(--color-border))]' 
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <button
              onClick={() => dispatch(toggleSidebarCollapse())}
              className="p-2 rounded-xl md:hidden hover:bg-[rgb(var(--color-surface-hover))] text-[rgb(var(--color-muted))] transition-colors"
              aria-label="Toggle menu"
            >
              <Menu size={20} />
            </button>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br from-[rgb(var(--color-primary))] to-[rgb(var(--color-secondary))] flex items-center justify-center shadow-lg">
                <Command size={16} className="text-white sm:w-5 sm:h-5" />
              </div>
              <div className="hidden sm:block">
                <h1 className="text-lg sm:text-xl font-semibold text-[rgb(var(--color-text))]">
                  Dashboard
                </h1>
                <p className="text-xs text-[rgb(var(--color-muted))]">Welcome back, Admin</p>
              </div>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <div ref={themeMenuRef} className="relative">
              <button
                ...
                aria-label="Toggle theme"
              >
                <CurrentThemeIcon size={18} className="sm:w-5 sm:h-5" />
                <span className="absolute top-1 right-1 w-1.5 h-1.5 bg-[rgb(var(--color-primary))] rounded-full animate-pulse" />
              </button>
              ...
              {showThemeMenu && (
                <div className={`
                  fixed sm:absolute
                  ${isMobile 
                    ? 'bottom-0 left-0 right-0 rounded-t-2xl animate-slideUp' 
                    : 'right-0 mt-2 rounded-2xl animate-scaleIn'
                  }
                  sm:absolute sm:right-0 sm:mt-2
                  bg-[rgb(var(--color-surface))] shadow-2xl border border-[rgb(var(--color-border))]
                  overflow-hidden z-50
                  ${isMobile ? 'max-h-[80vh]' : 'w-80'}
                `}>
                  {/* Header */}
                  <div className="p-4 border-b border-[rgb(var(--color-border))] bg-gradient-to-r from-[rgb(var(--color-primary)_/_0.05)] to-[rgb(var(--color-secondary)_/_0.05)]">
                    <div className="flex items-center justify-between">
                      ...
                      {isMobile && (
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-[rgb(var(--color-surface-hover))]"
                        >
                          <X size={18} />
                        </button>
                      )}
                      ...
                      {themes.map((theme) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => setTheme(theme.id)}
                            className={`
                              group relative flex flex-col items-center gap-2 p-2 sm:p-3 rounded-xl
                              transition-all duration-300
                              ${isActive 
                                ? 'bg-[rgb(var(--color-primary)_/_0.1)] ring-2 ring-[rgb(var(--color-primary))]' 
                                : 'hover:bg-[rgb(var(--color-surface-hover))]'
                              }
                            `}
                          >
                            ...
                            <div className="flex items-center gap-1">
                              <Icon size={10} />
                              <span className="text-[10px] sm:text-xs font-medium text-[rgb(var(--color-text))]">
                                {theme.name}
                              </span>
                            </div>
                          </button>
                  ...
                  <div className="p-3 border-t border-[rgb(var(--color-border))] bg-[rgb(var(--color-bg))]">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-[rgb(var(--color-muted))] flex items-center gap-1">
                        <span className="w-1.5 h-1.5 bg-[rgb(var(--color-success))] rounded-full animate-pulse" />
                        Theme saved
                      </span>
                      <span className="text-[rgb(var(--color-primary))] font-medium">
                        {themes.find(t => t.id === currentTheme)?.name}
            ...
            <div ref={notificationRef} className="relative">
              <button
                onClick={() => setShowNotifications(!showNotifications)}
                className="relative p-2 sm:p-2.5 rounded-xl hover:bg-[rgb(var(--color-surface-hover))] text-[rgb(var(--color-muted))] transition-colors"
                aria-label="Notifications"
              >
                <Bell size={18} className="sm:w-5 sm:h-5" />
                <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 bg-[rgb(var(--color-danger))] rounded-full ring-2 ring-[rgb(var(--color-surface))] animate-pulse" />
              </button>
              ...
              {showNotifications && (
                ...
                `}>
                  ...
                  <div className="max-h-[60vh] overflow-y-auto">
                    {[
                      { title: 'New user registered', time: '2 min ago', icon: '👤' },
                      { title: 'System update completed', time: '1 hour ago', icon: '⚡' },
                      { title: 'Weekly report ready', time: '3 hours ago', icon: '📊' },
                      { title: 'Payment received', time: '5 hours ago', icon: '💰' },
                      { title: 'New message from support', time: '1 day ago', icon: '💬' }
                    ].map((notification, i) => (
                      <div key={i} className="p-4 hover:bg-[rgb(var(--color-surface-hover))] transition-colors cursor-pointer border-b border-[rgb(var(--color-border))]">
                        <div className="flex items-start gap-3">
                          <div className="w-10 h-10 rounded-xl bg-[rgb(var(--color-primary)_/_0.1)] flex items-center justify-center text-xl">
                            {notification.icon}
                          </div>
                          <div className="flex-1">
                            <p className="text-sm font-medium text-[rgb(var(--color-text))]">
                  ...
                  <div className="p-3 border-t border-[rgb(var(--color-border))] bg-[rgb(var(--color-bg))]">
                    <button className="w-full py-2 text-sm text-[rgb(var(--color-primary))] font-medium text-center hover:bg-[rgb(var(--color-primary)_/_0.05)] rounded-lg transition-colors">
                      View all notifications
                    </button>
                  </div>
                </div>
            ...
            <div ref={userMenuRef} className="relative ml-1 sm:ml-2">
              <button
                onClick={() => setShowUserMenu(!showUserMenu)}
                className="flex items-center gap-2 sm:gap-3 pl-2 pr-2 sm:pr-3 py-1.5 rounded-xl hover:bg-[rgb(var(--color-surface-hover))] transition-colors group"
              >
                <div className="relative">
                  <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-gradient-to-br from-[rgb(var(--color-primary))] to-[rgb(var(--color-secondary))] flex items-center justify-center text-white font-semibold text-sm shadow-lg">
                    JD
                  </div>
                  <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-[rgb(var(--color-success))] border-2 border-[rgb(var(--color-surface))] rounded-full" />
                </div>
                <div className="hidden md:block text-left">
                  <p className="text-sm font-semibold text-[rgb(var(--color-text))]">John Doe</p>
                  <p className="text-xs text-[rgb(var(--color-muted))]">Administrator</p>
                </div>
                <ChevronDown size={16} className="text-[rgb(var(--color-muted))] hidden sm:block" />
              </button>
              ...
              {showUserMenu && (
                ...
                `}>
                  <div className="p-4 bg-gradient-to-r from-[rgb(var(--color-primary)_/_0.05)] to-[rgb(var(--color-secondary)_/_0.05)]">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[rgb(var(--color-primary))] to-[rgb(var(--color-secondary))] flex items-center justify-center text-white font-semibold text-lg shadow-lg">
                          JD
                        </div>
                        <div>
                          <p className="font-semibold text-[rgb(var(--color-text))]">John Doe</p>
